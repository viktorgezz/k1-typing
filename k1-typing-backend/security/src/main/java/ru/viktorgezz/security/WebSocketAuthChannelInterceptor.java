    ...
    private void authenticateUser(StompHeaderAccessor accessor) {
        ...
        try {
            ...
            if (usernameExtracted != null) {
                UserDetails userDetails = userDetailsService.loadUserByUsername(usernameExtracted);

                if (jwtService.validateToken(token, userDetails.getUsername(), usernameExtracted)) {
                    ...
                    log.debug("WebSocket authenticated user: {}", usernameExtracted);
                } else {
